...
public class CatchGradualDifficultyAttributes
@PublishedApi internal constructor(override val ptr: NativePointer, override val map: Beatmap) :
    GradualDifficultyAttributes(mode = GameMode.Catch) {
    ...
    protected override fun clone(): CatchGradualDifficultyAttributes {
        return CatchGradualDifficultyAttributes(ptr = clone(ptr = ptr), map = map)
    }

    override fun toString(): String = debug(ptr = ptr, pretty = pretty())

    override fun next(): CatchDifficultyAttributes {
        val next = next(ptr = ptr)
        if (next == 0L) throw NotImplementedError("return is null")
        return CatchDifficultyAttributes(ptr = next)
    }
