...
public class CatchGradualPerformanceAttributes
@PublishedApi internal constructor(override val ptr: NativePointer, override val map: Beatmap) :
    GradualPerformanceAttributes(mode = GameMode.Catch) {
    ...
    override fun processNext(state: ScoreState, number: Int): CatchPerformanceAttributes? {
        val next = next(ptr = ptr, state = state.ptr, number = number)
        if (next == 0L) return null
        return CatchPerformanceAttributes(ptr = next)
    }
