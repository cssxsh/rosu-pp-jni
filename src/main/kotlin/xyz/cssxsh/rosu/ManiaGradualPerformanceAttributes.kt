...
public class ManiaGradualPerformanceAttributes
@PublishedApi internal constructor(override val ptr: NativePointer, override val map: Beatmap) :
    GradualPerformanceAttributes(mode = GameMode.Mania) {
    ...
    override fun processNext(state: ScoreState, number: Int): ManiaPerformanceAttributes? {
        val next = next(ptr = ptr, state = state.ptr, number = number)
        if (next == 0L) return null
        return ManiaPerformanceAttributes(ptr = next)
    }
